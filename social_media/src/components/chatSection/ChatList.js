...
import { GET_USER_GROUPS, SEND_MESSAGE, SEND_MESSAGE_WITH_FILE, DELETE_MESSAGE, GET_MESSAGES, GET_ALL_USERS, GET_GROUP_UNREAD_COUNT, MARK_GROUP_MESSAGE_AS_READ, GET_UNREAD_COUNT, MARK_ALL_MESSAGES_AS_SEEN } from '../../graphql/mutations';
...
const ChatList = ({ activeTab, createdGroups }) => {
  ...
  }, []); // No dependencies needed for socket listeners
  ...
  const sendAudioMessage = async () => {
    ...
    try {
      ...
      if (result.success && result.media) {
        await sendMessageWithFileMutation({
          variables: {
            senderId: sender?.id,
            receiverId: selectedChat?.id,
