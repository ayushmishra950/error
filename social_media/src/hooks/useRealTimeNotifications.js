...
const useRealTimeNotifications = (userId) => {
  ...
  let incrementUnreadCount = () => {};
  
  try {
    const notificationContext = useNotifications();
    addNewNotification = notificationContext.addNewNotification || (() => {});
    incrementUnreadCount = notificationContext.incrementUnreadCount || (() => {});
  } catch (error) {
    ...
    try {
      ...
      socketRef.current.on('newNotification', (notification) => {
        ...
        if (showableTypes.includes(notification.type)) {
          ...
          console.log('❌ Notification type not supported for popup:', notification.type);
        }
      });
  ...
  const handleNewNotification = (notification) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('❌ Failed to update red dot:', error);
      }
  ...
  useEffect(() => {
    if (notificationsData?.getUserNotifications && !loading) {
      ...
      if (newNotifications.length > 0) {
        ...
        console.log('📊 Polling check: No new notifications');
      }
